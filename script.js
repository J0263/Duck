...
function getSrc(scrollPos) {
    ...
    const quarterHeight = .036 * viewportHeight;
    index = Math.ceil((scrollPos?scrollPos:1)/quarterHeight);
    console.log("mathed index:", index);
    if (index>=27) {
      pauseEventListener("scroll", scrollFunction, false);
    }
    else if (index==27) addEventListener, scrollFunction;
    return 'DuckAnimation' + index + '.webp';
  }
function scrollFunction(){
  const fileName = getSrc(window.scrollY)
  console.log(fileName)
  imageEl.src = './AnimationImages/' + fileName
}
